...
import { selectedDay as selectedDayState, multiDayEventsAtom } from "@root/Recoil/calendar.atom";
import Day from "@root/components/Day/Day";
import { getEventsForTheDate, stringTo2Digits } from "@root/libs/getEventsForTheDate";
...
export default function Week({
  ...
  const todayYear = today.getFullYear(); // console.log('Week events: ', events)
  ...
  const shouldEventShowInThisWeek = (eventFirstDay, eventLastDay, weekFirstDay, weekLastDay) => {
    // all four arguments are in this format '2021-05-01'
    const [eventFirst, eventLast, weekFirst, weekLast] = [new Date(eventFirstDay)?.getTime(), new Date(eventLastDay)?.getTime(), new Date(weekFirstDay)?.getTime(), new Date(weekLastDay)?.getTime()]; // console.log(eventFirstDay, weekLastDay)
  ...
  const getDateTimeStamp = dateObj => {
    ...
    return new Date(`${dateObj.year}-${stringTo2Digits(dateObj.month)}-${stringTo2Digits(dateObj.date)}`).getTime();
  };
  ...
  const barsToShowMultiDayEvents = (multiDayEventsArray, weekFirstDay, weekLastDay) => {
    ...
    multiDayEventsArray.forEach(element => {
      if (shouldEventShowInThisWeek(element.multi_day_first, element.multi_day_last, weekFirstDay, weekLastDay)) {
        const barPosition = barPositionInThisWeek(datesArray, element.multi_day_first, element.multi_day_last);
  ...
  React.useEffect(() => {
    ...
    const weekLastDay = `${lastDay.year}-${stringTo2Digits(lastDay.month)}-${stringTo2Digits(lastDay.date)}`; // console.log('First and Last day: ', firstDay, lastDay);
    // console.log('multidayEventsArr', multiDayEventsState)

    const arr = barsToShowMultiDayEvents(multiDayEventsState, weekFirstDay, weekLastDay);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: `${localMultiDayEvents?.length ? "block" : "hidden"} absolute top-9 md:top-10 w-full`
  }, localMultiDayEvents.map((x, index) => {
    return /*#__PURE__*/React.createElement(MultiDayEvent, {
      cellWidth: cellWidth,
    ...
    return /*#__PURE__*/React.createElement(Day // date={x.date}
    // month={x.month}
    , _extends({
      width: cellWidth,
      ...
      eventsProp: getEventsForTheDate(x.month, x.date, x.year, events),
      eventsGroupMarginTop: `${localMultiDayEvents.length ? "mt-" + localMultiDayEvents.length * 6 : ""}` // DONE: this margin top should change base on how many bars in this day block
