...
test("normal day component should have classes", () => {
  render( /*#__PURE__*/React.createElement(Day, {
    date: "1",
    month: "5",
    width: "120",
    isToday: false,
...
test("Recoil selectedDay default to be false, and update to object it received", () => {
  ...
  expect(initialSnapshot.getLoadable(selectedDayState).valueOrThrow()).toMatchObject({
  ...
  let testSnapshot;
  act(() => {
    testSnapshot = snapshot_UNSTABLE(({
      set
    }) => set(selectedDayState, {
      date: "3",
      month: "6",
      year: "2021"
    }));
  });
  expect(testSnapshot.getLoadable(selectedDayState).valueOrThrow()).toMatchObject({
    date: "3",
    month: "6",
    year: "2021"
  });
