...
import { calendarStart as calendarStartState, targetMonth as targetMonthState, targetYear as targetYearState } from "@root/Recoil/calendar.atom";
...
export default function CalendarView({
  ...
  React.useEffect(() => {
    setWeeksState(weeksGenerator(calendarStart, targetMonth, targetYear)); // console.log('Array', targetMonth, targetYear)
  }, [calendarStart, targetMonth, targetYear]);
  const calendarRef = React.useCallback(node => {
    if (!node) return;
    const {
      width
