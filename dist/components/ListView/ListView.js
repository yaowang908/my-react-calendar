...
import { targetYear as targetYearState, targetMonth as targetMonthState } from "@root/Recoil/calendar.atom";
...
export default function ListView({
  ...
  const shouldMultiDayEventShow = (startDateDetails, endDateDetails, targetMonth, targetYear) => {
    if (!(startDateDetails && endDateDetails && targetMonth && targetYear)) return false;
    const [startDateMonth, startDateYear] = [Number(startDateDetails?.month), Number(startDateDetails?.year)];
    const [endDateMonth, endDateYear] = [Number(endDateDetails?.month), Number(endDateDetails?.year)];
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    if (shouldMultiDayEventShow(x?.start_date_details, x?.end_date_details, targetMonth, targetYear)) {
    ...
    if (Number(x?.end_date_details?.year) === Number(targetYear) && Number(x?.end_date_details?.month) === Number(targetMonth)) {
      return /*#__PURE__*/React.createElement(ListEntry, {
        key: nanoid(),
